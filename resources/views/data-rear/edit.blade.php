    ...
    <div class="container shadow bg-grey text-align: center">
        <div class="row m-3">
            <div class="col-8 text-align: center">
                ...
                <div class="row">
                    <form class="{{ route('datarear.update', $datarear->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        <input type="hidden" id="id" name="id" value="{{ $datarear->id }}">
                        <div class="form-group mt-2">
                            <label for="kode" class="text-light">Kode</label>
                            <input type="text" id="kode" name="kode" class="form-control"
                                value="{{ $datarear->kode }}" autocomplete="off" required>
                        </div>
                        <div class="form-group mt-2">
                            <label for="model" class="text-light">Model</label>
                            <input type="text" id="model" name="model" class="form-control"
                                value="{{ $datarear->model }}" autocomplete="off" required>
                        </div>
                        <div class="form-group mt-2">
                            ...
                            <select class="form-control" name="id_gambar" id="id_gambar" required>
                                ...
                                @foreach ($displayrear as $index)
                                    <option value="{{ $index }}" @if ($index == $datarear->id_gambar) selected @endif>
                                        {{ $index }}</option>
                                @endforeach
                            </select>                   
                        </div>
