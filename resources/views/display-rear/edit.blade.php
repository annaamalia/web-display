    ...
    <div class="container shadow bg-grey ml-6 mr-6">
        <div class="row m-3">
            <div class="col-3">
                ...
                <div class="row">
                    <form class="{{ route('formrear.update', $displayrear->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        <input type="hidden" id="id" name="id" value="{{ $displayrear->id }}">
                        <br>
                        <div class="form-group mt-2">
                            <label for="id_gambar" class="text-light">ID Gambar</label>
                            <input type="text" id="id_gambar" name="id_gambar" class="form-control"
                            value="{{ $displayrear->id_gambar }}" autocomplete="off" required>
                        </div>
                        <div class="form-group mt-2">
                            ...
                            <input type="file" id="gambar" name="gambar" class="form-control"
                                value="{{ $displayrear->gambar }}" autocomplete="off" required onchange="previewImage()">
                        </div>                        
            ...
            <div class="col-9">
                <div id="id" name="id" value="{{ $displayrear->id }}">
                    <img img class="center-fit"  src="{{ asset('storage/' . $displayrear->gambar) }}">
                </div>
            </div>
        </div>
    </div>

@include('display-rear.script')